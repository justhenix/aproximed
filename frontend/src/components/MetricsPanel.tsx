    ...
    <div className="glass-card p-6 mt-6">
      ...
      {metrics && metrics.png_output_ratio < 1 && (
        <div className="mb-6 bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-3 rounded-xl flex items-start gap-3">
          ...
          <div className="text-sm">
            ...
            <p>PNG output is larger than the uploaded file. This can happen because SVD matrix compression and PNG byte compression are different.</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <MetricCard 
          ...
          helper="Rank k chosen vs recommended."
        />
        
        <MetricCard 
          title="SVD Energy Retained" 
          value={metrics ? `${(metrics.retained_energy * 100).toFixed(2)}%` : "-"} 
          helper="Energy preserved by selected singular values. Not the same as visual quality."
        />
        
        <MetricCard 
          title="SVD Matrix Ratio" 
          value={metrics ? `${metrics.svd_compression_ratio.toFixed(2)}x` : "-"} 
          helper="Theoretical low-rank matrix storage estimate."
        />
        
        <MetricCard 
          title="PNG Output Ratio" 
          value={metrics ? `${metrics.png_output_ratio.toFixed(2)}x` : "-"} 
          helper="Actual encoded PNG byte-size comparison."
        />

        <div className="sm:col-span-2 md:col-span-4 p-4 bg-white/60 rounded-2xl border border-gray-100 mt-2">
          <p className="text-sm font-semibold text-gray-700 mb-1">MSE / PSNR</p>
          ...
          <p className="text-xs text-gray-500 mt-2">Error metrics indicating visual reconstruction accuracy.</p>
        </div>
