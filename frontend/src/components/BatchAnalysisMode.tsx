    ...
    <div className="w-full flex flex-col items-center space-y-6 sm:space-y-8 max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 min-w-0">
      ...
      <div className="w-full max-w-6xl space-y-5 sm:space-y-6 min-w-0">
        <div className="glass-card p-4 sm:p-6 border border-gray-100 shadow-sm rounded-2xl bg-white/80 min-w-0">
          <div className="mb-4">
            <h3 className="text-base sm:text-lg font-semibold text-gray-900 text-balance">{language === 'id' ? 'Batch Manual (Gambar)' : 'Manual Batch (Images)'}</h3>
          ...
          <div className="mt-4 flex flex-col md:flex-row gap-4 md:items-end md:justify-between">
            <div className="w-full md:max-w-sm min-w-0">
              <label className="text-sm font-medium text-gray-700 block mb-1.5">Rank k = {batchRank}</label>
              <input
                type="range"
                min={1}
                max={200}
              ...
              {batchResult.per_image_results.map((item, index) => (
                ...
                >
                  <div className="flex items-start justify-between gap-2 min-w-0">
                    ...
                    <div className="shrink-0 text-xs">{renderStatus(item.status, language)}</div>
                  </div>
                  <div className="grid grid-cols-2 gap-x-3 gap-y-2 text-xs">
                    <div className="min-w-0">
                      <div className="text-[10px] uppercase tracking-wide text-gray-400 font-bold">{language === 'id' ? 'Asli' : 'Original'}</div>
                      <div className="font-mono text-gray-800 truncate">{formatFileSize(item.original_size_bytes)}</div>
                    </div>
                    <div className="min-w-0">
                      <div className="text-[10px] uppercase tracking-wide text-gray-400 font-bold">{language === 'id' ? 'Kompresi' : 'Compressed'}</div>
                      <div className="font-mono text-gray-800 truncate">{formatFileSize(item.compressed_size_bytes)}</div>
                    </div>
                    <div className="min-w-0">
                      <div className="text-[10px] uppercase tracking-wide text-gray-400 font-bold">{language === 'id' ? 'Hemat' : 'Saved'}</div>
                      <div className="font-mono text-gray-800 truncate">{formatFileSize(item.bytes_saved)}</div>
                    </div>
                    <div className="min-w-0">
                      <div className="text-[10px] uppercase tracking-wide text-gray-400 font-bold">{language === 'id' ? 'Reduksi' : 'Reduction'}</div>
                      <div className="font-mono text-gray-800 truncate">{formatPercent(item.size_reduction_pct)}</div>
                    </div>
                    <div className="min-w-0">
                      <div className="text-[10px] uppercase tracking-wide text-gray-400 font-bold">{language === 'id' ? 'Rasio' : 'Ratio'}</div>
                    ...
                    <div className="min-w-0">
                      <div className="text-[10px] uppercase tracking-wide text-gray-400 font-bold">{language === 'id' ? 'Energi' : 'Energy'}</div>
